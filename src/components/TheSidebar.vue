<template>
  ...
  <div class="sidebar">
    <el-menu class="sidebar-el-menu" :default-active="activeIndex" :collapse="collapse" unique-opened @select="selectFn">
      <template v-for="item in items">
          ...
          <el-submenu :index="item.name" :key="item.name">
            ...
            <template v-for="subItem in item.subs">
              <el-submenu v-if="subItem.subs" :index="subItem.name" :key="subItem.name">
                <template slot="title">{{ getTitle(subItem) }}</template> <!-- 三级 -->
        ...
        <template v-else>
          <el-menu-item :index="item.name" :key="item.name">
            <i :class="['iconfont','mr5',item.icon]"></i><span slot="title">{{ getTitle(item)}}</span> <!-- 一级 -->
          </el-menu-item>
        </template>
      </template>
    </el-menu>
...
<script>
  ...
  export default {
    ...
    created() {
      let that = this;
      // setTimeout(() => {
      that.items = [...items.items]
      this.$store.commit('SET_TABLE_BAR', that.items)
      // }, 100)
    ...
    computed: {
      ...
      barTitleFn() {
        return this.$store.getters.barTitle
      }
    },
    watch: {
      barTitleFn(n) {
