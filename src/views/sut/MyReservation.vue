                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                     <!-- 修改 -->
                      ...
                      <!-- 删除 -->
                      <el-tooltip class="item" effect="dark" :content="$t('Lang.Views.Sut.MyReservation.Cancel')" placement="top-start">
                      <el-popconfirm
...
export default {
    ...
    methods: {
      ...
      getResevaList(page, prop, order, keyword, type) { // 获取所有的列表
          Api.getReserList({page, prop, order, keyword, type, user_page: 'user_page'}).then((res) => {
              if (res.success === true) {
                this.tableData = res.data
                this.Allpage = res.all_page
                  if (this.PageIndex > 1 && this.tableData.length === 0) {
                    this.getResevaList(this.PageIndex - 1)
                  }
              }
          })
      },
      ...
      sutName(id) {
        this.$router.push({name: this.$t('Lang.module.Sidebr.Details'), query: {id}})
      },
